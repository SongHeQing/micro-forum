...
du -h                                      # 显示目录空间使用情况
du -sh /path                               # 显示目录总大小
du -sh *                                   # 查看当前目录下各子目录和文件的大小
du -sh .                                   # 查看当前目录总大小
du -sh * | sort -hr                        # 按大小排序显示文件和目录
```

### ls命令补充说明
- `ls -l`: 以长格式显示文件和目录的详细信息，文件大小以字节为单位显示
- `ls -lh`: 以长格式显示文件和目录的详细信息，文件大小以人类可读格式显示(K,M,G等)

**使用 `ls -l`:**
```
-rw-r--r-- 1 user group 1024000 Jan 1 12:00 example.txt
-rw-r--r-- 1 user group 2048000 Jan 1 12:00 largefile.zip
```

**使用 `ls -lh`:**
```
-rw-r--r-- 1 user group 1000K Jan 1 12:00 example.txt
-rw-r--r-- 1 user group 2.0M Jan 1 12:00 largefile.zip
```

在上面的例子中，`ls -l` 显示原始字节数(1024000, 2048000)，而 `ls -lh` 将这些数字转换为更易于理解的形式(1000K, 2.0M)。

所以，`ls -lh` 主要的优势是让文件大小更容易被人理解，特别是当文件很大时，不用自己去数零来判断文件的实际大小。


### df - 查看磁盘空间使用情况
```bash
df -h                                      # 查看所有挂载分区的使用情况
df -h 目录名                                # 查看指定目录所在分区的使用情况
```
